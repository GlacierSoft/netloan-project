			...
			for(RepaymentNotesDetail repaymentNotesDetail : repaymentNotesDetails){//改变还款记录和还款记录明细
			    ...
			    if(overDayReal > 0){//判断是否逾期
			    	...
			    	float currOverDueMoney = (overdueInterest - repaymentNotesDetail.getOverdueInterest())
			    							+(overdueManaFee - repaymentNotesDetail.getOverdueManaFee());//当期新增的逾期总额
			    	float currOverInterest = overdueInterest - repaymentNotesDetail.getOverdueInterest();//当期新增的逾期利息
			    	...
			    	if(count==1){
				    	...
				    	memberStatisticsRepay.setOverdueFineAmount(memberStatisticsRepay.getOverdueFineAmount()+currOverDueMoney);//设置逾期罚款总额
				    	memberStatisticsRepay.setOverdueInterestAmount(memberStatisticsRepay.getOverdueInterestAmount()+currOverInterest);//设置逾期利息
				    	memberStatisticsRepay.setWaitAlsoTotal(memberStatisticsRepay.getWaitAlsoTotal()+currOverDueMoney);//设置待还总额
				    	memberStatisticsRepay.setUpdateTime(new Date());
				    	//执行更新会员统计信息
				    	memberStatisticsMapper.updateByPrimaryKeySelective(memberStatisticsRepay);
			    		//更新会员资金的信息
				    	FinanceMember financeMember = financeMemberMapper.selectByMemberId(borrowingLoan.getMemberId());
				    	financeMember.setRefundMoney(financeMember.getRefundMoney()+currOverDueMoney);//设置会员资金待还金额
				    	financeMemberMapper.updateByPrimaryKeySelective(financeMember);//执行更新
				    }
			    }
			}
			for(ReceivablesNotesDetail receivablesNotesDetail : receivablesNotesDetails){//更新收款记录和收款记录明细
			    ...
			    if(overDayReal > 0){//判断是否逾期
			    	...
			    	float addOverdueInterest = overdueInterest - receivablesNotesDetail.getOverdueInterest();//新增逾期罚息=现在罚息-原来罚息
			    	...
			    	if (count == 1) {
			    		...
			    		memberStatisticsTender.setWaitIncomeTotal(memberStatisticsTender.getWaitIncomeTotal()+addOverdueInterest);//设置待收总额(原来的待收总额+新增罚息))
			    		//执行更新会员统计信息
				    	...
				    	financeMemberTender.setCollectingMoney(financeMemberTender.getCollectingMoney()+addOverdueInterest);//设置待收总额(原来的待收总额+新增罚息))
